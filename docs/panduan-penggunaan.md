...
4. Pilih salah satu metode restore berikut:

   **Opsi 1 (utama, lewat browser):**
   1. Buka menu **backup/restore** di server baru.
   2. Pilih file backup `.tar.gz` dari perangkat lokal.
   3. Tekan tombol **restore dari upload**.
   4. Konfirmasi restore dan tunggu notifikasi berhasil.

   **Opsi 2 (alternatif, file sudah ada di server):**
   1. Pastikan file backup tersedia di server baru pada lokasi backup aplikasi.
   2. Buka menu **backup/restore**.
   3. Tekan tombol **restore** untuk memakai file backup yang tersimpan di server.

> **Catatan teknis untuk administrator server:** Opsi 1 direkomendasikan untuk migrasi data existing karena tidak memerlukan copy manual file ke folder server.
